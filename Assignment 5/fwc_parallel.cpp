...
double checksum(World &world) {
    // 
    double cs=0;
    // TODO: make sure checksum is computed globally DONE
    // only loop *inside* data region -- not in ghostzones!
    
    // We dont change anything here, but we use MPI_reduce after the function is used in simulate
...
void stat(World &world) {
    // TODO: make sure stats are computed globally DONE
    double mint = 1e99;
    double maxt = 0;
    double meant = 0;
    for (uint64_t i = 1; i < world.latitude - 1; ++i)
    ...
    // Changed to the local latitude and longitude here, to get mean for that world
    meant = meant / (world.latitude * world.longitude);
    
    // Make pointers to somewhere we can store the global values (sum of locals)
    // POTENTIAL BUG SHOULD IT BE THE VALUE ZERO HERE
    double global_mint;
    double global_maxt;
    double global_meant;
    
    // Sum the local values from the individual worlds on one rank
    // Different operations described here: https://mpitutorial.com/tutorials/mpi-reduce-and-allreduce/
    MPI_Reduce( &mint, &global_mint, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD );
    MPI_Reduce( &maxt, &global_maxt, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD );
    MPI_Reduce( &meant, &global_meant, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD );
    // Divide sum by numer of ranks to obtain avg
    // THIS ONLY WORKS FOR AN MPI_SIZE THAT LONGITUDE CAN BE DIVIDED BY WITH 0 AND GIVE AN INTEGER
    global_meant /= mpi_size;
...
void exchange_ghost_cells(World &world) {
    // TODO: figure out exchange of ghost cells bewteen ranks DONE
    
    // VERTICAL GHOST CELLS ---------------------
    // Define what to send out with the proper length of the column
    std::vector<double> emit_left( world.latitude );
    std::vector<double> emit_right( world.latitude );
    // Fill them out (loop over rows)
    ...
    // Define places to store what is received (with length of column)
    std::vector<double> recv_left( world.latitude );
    std::vector<double> recv_right( world.latitude );
    
    // Make a reuest vector
    std::vector<MPI_Request> req(4);
    
    // Send out (tag 0 denotes send to left, tag 1 denotes send to right)
    int right_rank = (mpi_rank + 1) % mpi_size;
    MPI_Isend(&emit_right[0], world.latitude, MPI_DOUBLE, right_rank, 1, MPI_COMM_WORLD, &req[0]);
    int left_rank = (mpi_size + mpi_rank - 1) % mpi_size;
    MPI_Isend(&emit_left[0], world.latitude, MPI_DOUBLE, left_rank, 0, MPI_COMM_WORLD, &req[1]);
    
    // Receive (tag 0 denotes receive from left (store in right), tag 1 denotes receive from right (store in left) )
    MPI_Irecv(&recv_right[0], world.latitude, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD, &req[2]);
    MPI_Irecv(&recv_left[0], world.latitude, MPI_DOUBLE, left_rank, 1, MPI_COMM_WORLD, &req[3]);
    
    
    // Wait for all communication
    MPI_Waitall(4, &req[0], MPI_STATUSES_IGNORE);
    
    // Store the received data in the right places 
    // (they are stored in ghost cells so they are shifted by 1 the extract to the emit columns)
    for (uint64_t i = 0; i < world.latitude; ++i) {
        world.data[i*world.longitude + world.longitude-1] = recv_right[i];
        world.data[i*world.longitude + 0] = recv_left[i];    
    }
    
    // HORIZONTAL GHOST CELLS ---------------------
    // This is the same as sequential since it exchanges with itself, due to the layout of this slab decomposition
...
void simulate(uint64_t num_of_iterations, const std::string& model_filename, const std::string& output_filename) {
    ...
    for (uint64_t iteration=0; iteration < num_of_iterations; ++iteration) {
        ...
        // TODO: gather the Temperature on rank zero DONE
        ...
        // MPI_Gather(&world.data[0], world.data.size(), MPI_DOUBLE, total_data.data(), world.data.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
        ...
        //        long int offset_lon = -1 + r*loc_x_size;
        //        for (uint64_t i = 1; i < latitude-1; ++i)
        //        for (uint64_t j = 1; j < longitude-1; ++j) {
        //
        //          uint64_t k_global = (i + offset_latitude) * global_world.longitude
    ...
    // Compute checksum for local world
    double cs = checksum(world);
    
    // Make a pointer to where we can store the total checksum
    double global_cs;
    
    // Collect the total checksum on rank 0, with the operator MPI_SUM
    MPI_Reduce( &cs, &global_cs, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD );
    
    // Only rank 0 should print results
    if (mpi_rank == 0) {
        std::cout << "checksum      : " << global_cs << std::endl;
